...
function buildJsonInput(json) {
    const plane = (json.plane ?? "XY");
    if (plane !== "XY" && plane !== "XZ" && plane !== "YZ") {
        console.error('Error: plane must be one of XY, XZ, YZ');
        process.exit(1);
    }
    const type = (json.type ?? "Mix");
    if (type !== "Mix" && type !== "OPO") {
        console.error('Error: type must be one of Mix, OPO');
        process.exit(1);
    }
...
async function main() {
    ...
    if (args["add-crystal"] !== undefined) {
        ...
        if (data.name) {
            CrystalDB.registerFromJson(data);
            const nl = data.nlData;
            if (nl) {
            ...
            for (const [name, def] of Object.entries(data)) {
                const d = def;
                CrystalDB.registerFromJson({ name, ...d });
                const nl = d.nlData;
                if (nl) {
    ...
    if (args.info !== undefined) {
        ...
        if (kindLabel)
            console.log(`  Type: ${kindLabel}`);
        if (info.crystalClass)
            console.log(`  Class: ${info.crystalClass}`);
        if (info.wavelengthRangeNm)
            console.log(`  Wavelength range: ${info.wavelengthRangeNm.join(" - ")} nm`);
        if (info.referenceCitations?.refractiveIndex)
            console.log(`  Refractive index source: ${info.referenceCitations.refractiveIndex}`);
        if (info.referenceCitations?.thermoOptic)
            console.log(`  Thermo-optic source: ${info.referenceCitations.thermoOptic}`);
        if (info.referenceCitations?.dTensor)
            console.log(`  d-tensor source: ${info.referenceCitations.dTensor}`);
        if (info.referenceCitations?.transmission)
            console.log(`  Transmission source: ${info.referenceCitations.transmission}`);
        if (info.thermalProperties?.densityKgPerM3)
            console.log(`  Density: ${info.thermalProperties.densityKgPerM3} kg/m^3`);
        if (info.thermalProperties?.specificHeatJoulePerKgK)
            console.log(`  Specific heat: ${info.thermalProperties.specificHeatJoulePerKgK} J/kg-K`);
        if (info.thermalProperties?.conductivityWattPerMeterK)
            console.log(`  Thermal conductivity: ${JSON.stringify(info.thermalProperties.conductivityWattPerMeterK)} W/m-K`);
        if (info.thermalProperties?.expansionCoefficients10Per6K)
            console.log(`  Thermal expansion: ${JSON.stringify(info.thermalProperties.expansionCoefficients10Per6K)} 1e-6/K`);
        process.exit(0);
    }
