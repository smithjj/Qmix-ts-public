...
try {
    ...
    transmissionData = JSON.parse(readFileSync(fixturePath, "utf8"));
}
catch {
...
function resolveCrystalName(name) {
    if (CrystalDB.hasCrystal(name))
        return name;
    for (const suffix of variantSuffixes) {
        if (name.endsWith(suffix)) {
            const base = name.slice(0, -suffix.length);
            if (CrystalDB.hasCrystal(base))
                return base;
        }
    }
    return name;
}
function opticalSign(name) {
    try {
...
function toLegacyCrystalInfo(info) {
...
app.post("/api/calculate", (req, res) => {
    try {
        ...
        const input = {
            ...
            principalPlane: (String(body.plane ?? "XY")),
            type: (String(body.type ?? "Mix")),
        };
        const engine = new QmixEngine();
        const customCrystals = body.customCrystals;
        if (customCrystals) {
...
app.post("/api/refractive-index", (req, res) => {
    try {
        ...
        const result = { indices, numPolarizations: indices.length };
        if (indices.length === 2) {
            const no = indices[0];
            const ne = indices[1];
            result.no = no;
