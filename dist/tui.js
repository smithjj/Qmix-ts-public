import { createCliRenderer, Box, Text, ScrollBox, BoxRenderable, TextRenderable, InputRenderable, SelectRenderable, SelectRenderableEvents, InputRenderableEvents, } from "@opentui/core";
...
const CRYSTAL_INFO = JSON.parse(readFileSync(crystalInfoPath, "utf8"));
/* ────────────────────────  helpers  ──────────────────────── */
const label = (t) => Text({ content: t, fg: "#888888" });
...
function crystalOptions(filter) {
    ...
    const names = q
        ...
        : CRYSTALS;
    if (names.length === 0)
        return [{ name: "No matches", description: "", value: "" }];
    return names.map((name) => ({ name, description: fmtValue(CRYSTAL_INFO[name]?.crystal_description), value: name }));
}
function updateCrystalSelect(select, filter, preferred = "BBO") {
    const current = select.getSelectedOption()?.value || preferred;
...
function crystalInfoText(code) {
    if (!code)
        return "No matching crystal selected. Refine the crystal search.";
    ...
    if (info.d_source)
        lines.push(`  nonlinear d:      ${fmtValue(info.d_source)}`);
    lines.push("");
    if (info.d_eff1 || info.d_eff2) {
        lines.push("d_eff formulae:");
        if (info.d_eff1)
            lines.push(`  d_eff1 = ${fmtValue(info.d_eff1)}`);
        if (info.d_eff2)
            lines.push(`  d_eff2 = ${fmtValue(info.d_eff2)}`);
        lines.push("");
    }
    if (info.d_string) {
        lines.push("d tensor:");
    ...
    if (info.thermal_conductivity_source)
        lines.push(`    ${fmtValue(info.thermal_conductivity_source).trim()}`);
    lines.push(`  expansion:    ${fmtValue(info.thermal_expansion)} ppm/K`);
    if (info.thermal_expansion_source)
        lines.push(`    ${fmtValue(info.thermal_expansion_source).trim()}`);
    lines.push(`  specific heat: ${fmtValue(info.specific_heat)} J/kg/K`);
    if (info.specific_heat_source)
        lines.push(`    ${fmtValue(info.specific_heat_source).trim()}`);
...
export async function startTui() {
    ...
    const outWrap = Box({ flexGrow: 1, flexShrink: 1, borderStyle: "single", title: " Output ", padding: 1 }, ScrollBox({
        width: "100%",
        height: "100%",
        scrollY: true,
        scrollX: false,
    ...
    const refInputs = [rSearch, rCry, rTmp, rWav, rTh, rPh];
    function calcRef() {
        try {
            const cry = (rCry.getSelectedOption()?.value ?? "BBO").toUpperCase().trim();
    ...
    const refScreen = Box({ id: "ref-scr", flexDirection: "column", flexGrow: 1 }, Box({ flexDirection: "row", gap: 1, marginBottom: 1, flexShrink: 0 }, Box({ flexDirection: "column", width: 24 }, label("Crystal search"), rSearch, rCry), Box({ flexDirection: "column", width: 12 }, label("Temp K"), rTmp), Box({ flexDirection: "column", width: 12 }, label("λ nm"), rWav), Box({ flexDirection: "column", width: 12 }, label("θ °"), rTh), Box({ flexDirection: "column", width: 12 }, label("φ °"), rPh)), outWrap);
    /* ══════════════════════  Qmix screen  ══════════════════════ */
    let qmixIdx = 0;
    ...
    const qmixInputs = [qSearch, qCry, qTmp, qR1, qR2, qBl, qPl, qTy];
    function showQmixCrystalInfo() {
        const cry = (qCry.getSelectedOption()?.value ?? "BBO").toUpperCase().trim();
    ...
    function calcQmix() {
        try {
            ...
            const pl = qPl.getSelectedOption()?.value || "XY";
            const ty = (qTy.getSelectedOption()?.value || "Mix");
            if (!CRYSTALS.includes(cry)) {
                setOut(`Unknown crystal: ${cry}\nKnown: ${CRYSTALS.slice(0, 20).join(", ")}…`);
                return;
            }
    ...
    const qmixScreen = Box({ id: "qmix-scr", flexDirection: "column", flexGrow: 1 }, Box({ flexDirection: "row", gap: 1, marginBottom: 1, flexShrink: 0 }, Box({ flexDirection: "column", width: 24 }, label("Crystal search"), qSearch, qCry), Box({ flexDirection: "column", width: 12 }, label("Temp K"), qTmp), Box({ flexDirection: "column", width: 10 }, label("Plane"), qPl), Box({ flexDirection: "column", width: 12 }, label("Type"), qTy)), Box({ flexDirection: "row", gap: 1, marginBottom: 1, flexShrink: 0 }, Box({ flexDirection: "column", width: 12 }, label("Red1 nm"), qR1), Box({ flexDirection: "column", width: 12 }, label("Red2 nm"), qR2), Box({ flexDirection: "column", width: 12 }, label("Blue nm"), qBl)), outWrap);
    /* ═══════════════════  Root layout  ═══════════════════ */
    const content = new BoxRenderable(renderer, { id: "content", flexDirection: "column", flexGrow: 1 });
    let currentScreen = "qmix";
    content.add(qmixScreen);
    renderer.root.add(Box({ flexDirection: "column", width: "100%", height: "100%", padding: 1 }, Box({ flexDirection: "row", gap: 2, height: 1, marginBottom: 1, flexShrink: 0 }, Text({ content: "[F1] Ref.Index   [F2] Qmix   [Tab] Next   [Enter] Calc   [Ctrl+C] Exit", fg: "#888888" })), content));
    /* ═══════════════════  Focus helpers  ═══════════════════ */
    function whichScreen() {
        return currentScreen;
    }
    function focusRef(i) { refIdx = ((i % refInputs.length) + refInputs.length) % refInputs.length; refInputs[refIdx].focus(); }
    function focusQmix(i) { qmixIdx = ((i % qmixInputs.length) + qmixInputs.length) % qmixInputs.length; qmixInputs[qmixIdx].focus(); }
    function nextField() {
        if (whichScreen() === "ref")
            focusRef(refIdx + 1);
        else
    ...
    function switchTo(screen) {
        if (currentScreen === screen)
            return;
        if (content.getRenderable("ref-scr"))
            content.remove("ref-scr");
        if (content.getRenderable("qmix-scr"))
            content.remove("qmix-scr");
