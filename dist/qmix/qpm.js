...
export function listQpmCrystals() {
    return Object.keys(QPM_DATA).sort();
}
export function getQpmPolarizations(crystal) {
    const entry = QPM_DATA[crystal];
    if (!entry)
        return [];
    return entry.polarizations;
}
function axisIndex(numPol, ch) {
    switch (ch) {
        case "X": return 0;
...
function rc(crystal, temp, wl, axis) {
    const v = CrystalDB.compute(crystal, temp, wl);
    return v[axis] ?? 0;
}
export function calculateQpm(crystal, temperatureKelvin, pumpNm, signalNm, idlerNm) {
    const entry = QPM_DATA[crystal];
    if (!entry)
        throw new Error(`QPM crystal ${crystal} not found`);
    const results = [];
    ...
    for (let p = 0; p < entry.polarizations.length; p++) {
        ...
        if (nPumpVec.some((v) => v === 0) || nSigVec.some((v) => v === 0) || nIdlVec.some((v) => v === 0))
            continue;
        const np = nPumpVec[iZ];
        const ns = nSigVec[iZ];
        const ni = nIdlVec[iZ];
        const deltaKOver2Pi = np / pumpNm - ns / signalNm - ni / idlerNm;
        if (Math.abs(deltaKOver2Pi) < 1e-20)
            continue;
 ...
 */
export function qpmSweep(crystal, temperatureKelvin, pumpNm, red1Nm, red2Nm, polIndex) {
    const entry = QPM_DATA[crystal];
    if (!entry)
        throw new Error(`QPM crystal ${crystal} not found`);
    const pol = entry.polarizations[polIndex];
    if (!pol)
        throw new Error(`Polarization index ${polIndex} out of range`);
    ...
    const range = type1
        ? [Math.min(blue, 1 / (1 / lambdaP - 1 / red)), lambdaP * 2]
        : [Math.min(blue, 1 / (1 / lambdaP - 1 / red)), Math.max(red, 1 / (1 / lambdaP - 1 / blue))];
    ...
    for (let i = 0; i < npts; i++) {
        ...
        if (!Number.isFinite(lambdaS) || lambdaS <= 0)
            continue;
        if (lambdaS < 0.9 * lambdaP)
            continue;
 ...
 */
function qpmFindSignal(crystal, temperatureKelvin, pumpNm, red1Nm, red2Nm, periodTargetUm, polIndex) {
    const entry = QPM_DATA[crystal];
    if (!entry)
        return undefined;
    const pol = entry.polarizations[polIndex];
    ...
    const transRangeWn = [1e7 / Math.max(red1Nm, red2Nm), 1e7 / Math.min(red1Nm, red2Nm)];
    ...
    if (type1)
        omegaBlue = Math.min(omegaBlue, 0.5 * omegaP);
...
export function qpmTempTune(crystal, pumpNm, red1Nm, red2Nm, periodUm, polIndex, tempMin, tempMax, tempSteps) {
    const results = [];
    for (let i = 0; i <= tempSteps; i++) {
        ...
        if (found)
            results.push({ param: temp, ...found });
    }
    return results;
}
/** Pump tuning: sweep pump wavelength at a fixed period and temperature using root-finding. */
export function qpmPumpTune(crystal, temperatureKelvin, pumpMinNm, pumpMaxNm, red1Nm, red2Nm, periodUm, polIndex, pumpSteps) {
    const results = [];
    for (let i = 0; i <= pumpSteps; i++) {
        ...
        if (found)
            results.push({ param: pump, ...found });
    }
    return results;
}
function firstSquaredIndex(crystal, wavelengthNm) {
