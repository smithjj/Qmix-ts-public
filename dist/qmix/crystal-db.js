...
for (const [name, entry] of Object.entries(entries)) {
    const inline = BUILT_IN_CRYSTAL_METADATA[name];
    if (inline !== undefined) {
        ...
        entries[name] = {
            ...
            metadata: {
                ...
                wavelengthRangeNm: [entry.rangeNm[0], entry.rangeNm[1]],
...
export class CrystalDB {
     ...
     */
    static compute(crystalName, temperatureKelvin, wavelengthNm) {
    ...
    static getCrystalInfo(crystalName) {
    ...
    static registerFromJson(json) {
        ...
        const tempCorrection = Array.isArray(tempCoeffs) && tempCoeffs.length === numPol
            ...
            : undefined;
        const metadata = json.description || json.formula || json.crystalClass || json.referenceCitations || json.thermalProperties
            ? {
                name,
                kind: numPolarizationsToKind(numPol),
                wavelengthRangeNm: [range[0], range[1]],
                description: json.description,
                formula: json.formula,
                crystalClass: json.crystalClass,
                referenceCitations: json.referenceCitations,
    ...
    static register(def) {
        ...
        if (nonlinear !== undefined) {
            if (kind === "uniaxial" || kind === "isotropic") {
                registerUniaxialNlData(name, nonlinear);
            }
            else {
                registerBiaxialNlData(name, nonlinear);
            }
        }
    }
    ...
    static unregister(crystalName) {
        if (builtInEntries.has(crystalName))
            return false;
        if (entries[crystalName] === undefined)
            return false;
        delete entries[crystalName];
        return true;
    }
}
function numPolarizationsToKind(numPolarizations) {
    if (numPolarizations === 1)
        return "isotropic";
    if (numPolarizations === 2)
        return "uniaxial";
    return "biaxial";
}
function kindToNumPolarizations(kind) {
    switch (kind) {
...
function ensureNotRegistered(name) {
    if (entries[name] !== undefined) {
        throw new Error(`Crystal '${name}' is already registered`);
    }
}
function getEntry(crystalName) {
    const entry = entries[crystalName];
    if (entry === undefined) {
...
function makeEntry(sellmeierFunction, numPolarizations, rangeNm, coeffs, tempCorrection) {
    return {
        kind: "generic",
        sellmeierFunction,
        numPolarizations,
...
function makeCustomEntry(compute, numPolarizations, rangeNm) {
    return {
        kind: "custom",
        compute,
        numPolarizations,
...
function computeGenericEntry(entry, temperatureKelvin, wavelengthNm) {
...
function genericSellmeier(sellmeierFunction, coeff, lambda2) {
...
function temperatureCorrection(tempFunction, coeff, lambda, lambda2, dT) {
    switch (tempFunction) {
        case 1:
            return 1e-5 *
...
function computeAda(temperatureKelvin, wavelengthNm) {
    const lambda = wavelengthNm * 1e-3;
    const lambda2 = lambda ** 2;
    const rows = [
...
function computeAdp(temperatureKelvin, wavelengthNm) {
    const lambda = wavelengthNm * 1e-3;
    const lambda2 = lambda ** 2;
    const rows = [
    ...
    return rows.map((row) => Math.sqrt(Math.abs(row.A + row.a * temperatureKelvin +
...
function computeAgs(temperatureKelvin, wavelengthNm) {
    const lambda = wavelengthNm * 1e-3;
    const lambda2 = lambda ** 2;
    const base = [
...
function computeAgse(temperatureKelvin, wavelengthNm) {
    const lambda = wavelengthNm * 1e-3;
    const lambda2 = lambda ** 2;
    const rows = [
...
function computeB2ggs(temperatureKelvin, wavelengthNm) {
    const lambda = wavelengthNm * 1e-3;
    const lambda2 = lambda ** 2;
    const rows = [
...
function computeBgs(temperatureKelvin, wavelengthNm) {
    const lambda = wavelengthNm * 1e-3;
    const lambda2 = lambda ** 2;
    const rows = [
...
function computeBggse(temperatureKelvin, wavelengthNm) {
    const lambda = wavelengthNm * 1e-3;
    const lambda2 = lambda ** 2;
    const rows = [
...
function computeBgse(temperatureKelvin, wavelengthNm) {
    const lambda = wavelengthNm * 1e-3;
    const lambda2 = lambda ** 2;
    const rows = [
...
function computeBibo(temperatureKelvin, wavelengthNm) {
    const lambda = wavelengthNm * 1e-3;
    const lambda2 = lambda ** 2;
    const rows = [
...
function computeCda(temperatureKelvin, wavelengthNm) {
...
function computeCdse(temperatureKelvin, wavelengthNm) {
    const lambda = wavelengthNm * 1e-3;
    const lambda2 = lambda ** 2;
    const rows = [
...
function computeClbo(temperatureKelvin, wavelengthNm) {
    const lambda = wavelengthNm * 1e-3;
    const lambda2 = lambda ** 2;
    const rows = [
...
function computeCspOld(temperatureKelvin, wavelengthNm) {
    const lambda = wavelengthNm * 1e-3;
    const lambda2 = lambda ** 2;
    const rows = [
...
function computeCsp(temperatureKelvin, wavelengthNm) {
    const lambda = wavelengthNm * 1e-3;
    const lambda2 = lambda ** 2;
    const rows = [
...
function computeDada(_temperatureKelvin, wavelengthNm) {
    return sell2(wavelengthNm, [
...
function computeDadp(_temperatureKelvin, wavelengthNm) {
    return sell2(wavelengthNm, [
        { a: 2.279481, b: 1.215879, c: 57.97555433, d: 0.010761, e: 0.013262977 },
        { a: 2.151161, b: 1.199009, c: 126.6005279, d: 0.009652, e: 0.009712103 }
    ]);
}
function computeDcda(temperatureKelvin, wavelengthNm) {
    const temp = [-2.33e-5, -1.67e-5];
    return sell2(wavelengthNm, [
...
function computeDkdp(temperatureKelvin, wavelengthNm) {
    const lambda = wavelengthNm * 1e-3;
    const lambda2 = lambda ** 2;
    const z = (temperature, index) => {
        const rows = [
...
function computeDlap(temperatureKelvin, wavelengthNm) {
...
function computeGap(temperatureKelvin, wavelengthNm) {
    const lambda2 = (wavelengthNm * 1e-3) ** 2;
...
function computeGs(temperatureKelvin, wavelengthNm) {
    const lambda = wavelengthNm * 1e-3;
    const lambda2 = lambda ** 2;
    const rows = [
...
function computeKabo(temperatureKelvin, wavelengthNm) {
    const lambda = wavelengthNm * 1e-3;
    const lambda2 = lambda ** 2;
...
function computeKbbf(_temperatureKelvin, wavelengthNm) {
    const lambda2 = (wavelengthNm * 1e-3) ** 2;
    return [
...
function computeKbo(_temperatureKelvin, wavelengthNm) {
    const lambda2 = (wavelengthNm * 1e-3) ** 2;
    const rows = [
...
function computeKda(temperatureKelvin, wavelengthNm) {
    const temp = [-3.95e-5, -2.27e-5];
    return sell2(wavelengthNm, [
...
function computeKdp(temperatureKelvin, wavelengthNm) {
    const lambda2 = (wavelengthNm * 1e-3) ** 2;
    const rows = [
    ...
    return rows.map((row) => Math.sqrt(Math.abs(row.a + row.b * temperatureKelvin +
...
function computeKnbo3(temperatureKelvin, wavelengthNm) {
    const lambda = wavelengthNm * 1e-3;
    const lambda2 = lambda ** 2;
...
function computeKta2(temperatureKelvin, wavelengthNm) {
    const lambda = wavelengthNm * 1e-3;
    const rows = [
...
function computeKta1(temperatureKelvin, wavelengthNm) {
    const lambda = wavelengthNm * 1e-3;
    const lambda2 = lambda ** 2;
    const rows = [
...
function computeKta3(temperatureKelvin, wavelengthNm) {
    const lambda = wavelengthNm * 1e-3;
    const lambda2 = lambda ** 2;
    const rows = [
...
function computeKtpf(temperatureKelvin, wavelengthNm) {
    const lambda = wavelengthNm * 1e-3;
    const lambda2 = lambda ** 2;
...
function computeKtph(temperatureKelvin, wavelengthNm) {
    const lambda = wavelengthNm * 1e-3;
    const lambda2 = lambda ** 2;
    const rows = ktpTempRows([
...
function computeKtpk(temperatureKelvin, wavelengthNm) {
    const lambda = wavelengthNm * 1e-3;
    const lambda2 = lambda ** 2;
    const rows = [
...
function computeLbgo(temperatureKelvin, wavelengthNm) {
    const lambda = wavelengthNm * 1e-3;
    const lambda2 = lambda ** 2;
    const rows = [
...
function computeLfm(_temperatureKelvin, wavelengthNm) {
    const lambda2 = (wavelengthNm * 1e-3) ** 2;
    const rows = [
...
function computeLgs(temperatureKelvin, wavelengthNm) {
    const lambda = wavelengthNm * 1e-3;
    const lambda2 = lambda ** 2;
    const rows = [
...
function computeLiio3(temperatureKelvin, wavelengthNm) {
    const lambda2 = (wavelengthNm * 1e-3) ** 2;
    const rows = [
...
function computeLis(temperatureKelvin, wavelengthNm) {
...
function computeLise(temperatureKelvin, wavelengthNm) {
...
function computeLnbc(temperatureKelvin, wavelengthNm) {
    const lambda2 = (wavelengthNm * 1e-3) ** 2;
    const rows = [
...
function computeLnbm(temperatureKelvin, wavelengthNm) {
    const lambda2 = (wavelengthNm * 1e-3) ** 2;
...
function computeLnbs(temperatureKelvin, wavelengthNm) {
    const lambda2 = (wavelengthNm * 1e-3) ** 2;
    const rows = [
...
function computeLitam(temperatureKelvin, wavelengthNm) {
    const lambda2 = (wavelengthNm * 1e-3) ** 2;
...
function computeRbbf(_temperatureKelvin, wavelengthNm) {
    const lambda2 = (wavelengthNm * 1e-3) ** 2;
    return [
...
function computeRda(temperatureKelvin, wavelengthNm) {
    const temp = [3.37e-5, 2.21e-5];
    return sell2(wavelengthNm, [
...
function computeRdp(temperatureKelvin, wavelengthNm) {
    const temp = [3.74e-5, 2.73e-5];
    return sell2(wavelengthNm, [
...
function computeRta(temperatureKelvin, wavelengthNm) {
    const lambda = wavelengthNm * 1e-3;
    const lambda2 = lambda ** 2;
    const rows = [
...
function computeRtp(temperatureKelvin, wavelengthNm) {
    const lambda2 = (wavelengthNm * 1e-3) ** 2;
    const dT = temperatureKelvin - 273;
    const baseRows = [
    ...
    return baseRows.map((row) => {
        ...
        const out = [0, 1, 2].map((index) => requiredValue(row.aa[index], `RTP aa ${index}`) +
...
function computeSc4h(_temperatureKelvin, wavelengthNm) {
    const lambda2 = (wavelengthNm * 1e-3) ** 2;
    return [
...
function computeSc6h(_temperatureKelvin, wavelengthNm) {
    const lambda2 = (wavelengthNm * 1e-3) ** 2;
    return [
...
function computeTas(temperatureKelvin, wavelengthNm) {
    const lambda2 = (wavelengthNm * 1e-3) ** 2;
    const rows = [
...
function computeYcob(temperatureKelvin, wavelengthNm) {
    const lambda = wavelengthNm * 1e-3;
    const lambda2 = lambda ** 2;
    const rows = [
...
function computeZgp(temperatureKelvin, wavelengthNm) {
    const lambda2 = (wavelengthNm * 1e-3) ** 2;
    const rows = [
...
function computeZnse(temperatureKelvin, wavelengthNm) {
    const lambda2 = (wavelengthNm * 1e-3) ** 2;
    const coeffs = [
...
function computeBbo3(temperatureKelvin, wavelengthNm) {
    const lambda = wavelengthNm * 1e-3;
    const lambda2 = lambda ** 2;
    const coefficients = [
    ...
    return coefficients.map((coefficient) => {
        const n2 = Math.abs(coefficient.a +
        ...
        const thermalShift = (temperatureKelvin - 293) *
...
function computeLbo(temperatureKelvin, wavelengthNm) {
    const lambda = wavelengthNm * 1e-3;
    const lambda2 = lambda ** 2;
    const coefficients = [
...
function isInRange(wavelengthNm, rangeNm) {
    if (rangeNm.length === 0) {
        return true;
    }
...
function sell2(wavelengthNm, rows) {
    const lambda2 = (wavelengthNm * 1e-3) ** 2;
    return rows.map((row) => Math.sqrt(Math.abs(row.a + (row.b * lambda2) / (lambda2 - row.c) + row.d / (lambda2 - row.e))));
}
function ktpTempRows(base) {
    const thermal = [
...
function ktpThermal(row, lambda, lambda2, temperatureKelvin) {
    return (row.E + row.F / lambda + row.G / lambda2 + row.H / lambda ** 3) * (temperatureKelvin - 298);
}
function computeTernaryThermal(temperatureKelvin, wavelengthNm, referenceKelvin, rows) {
    const lambda = wavelengthNm * 1e-3;
    const lambda2 = lambda ** 2;
    return rows.map((row) => Math.sqrt(Math.abs(row.A + row.B / (lambda2 - row.C) + row.D / (lambda2 - row.E))) +
...
//# sourceMappingURL=crystal-db.js.map