import type { MixingType, QmixResult } from "./types.js";

export function formatQmixResults(
  results: readonly QmixResult[],
  mixType: MixingType
): string[] {
  return results.flatMap((result) => formatQmixResult(result, mixType));
}

export function formatQmixResult(result: QmixResult, mixType: MixingType): string[] {
...
function formatMinimalResult(result: QmixResult): string[] {
  return [
...
function interactionLine(lambda: readonly [number, number, number], polar: string): string {
  return `    ${fixed(lambda[0], 2, 7)}(${polar[0]}) + ${fixed(lambda[1], 2, 7)}(${polar[1]})  = ${fixed(lambda[2], 2, 7)}(${polar[2]})`;
}

function temperatureLine(value: number): string {
...
function fixed(value: number, digits: number, width: number): string {
...
function exponential(value: number, digits: number, width: number): string {
  const normalized = Object.is(value, -0) ? -0 : value;
  const text = normalized
    .toExponential(digits)
    .replace("e", "E")
    .replace(/E([+-])(\d+)$/, (_match, sign: string, exponent: string) => {
